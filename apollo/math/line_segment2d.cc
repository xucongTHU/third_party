...
LineSegment2d::LineSegment2d(const Vec2d &end, 
    const double &heading, const double &length)
    : end_(end), heading_(heading), length_(length) {
  start_.set_x(end_.x() - length * cos(heading_));
  start_.set_y(end_.y() - length * sin(heading_));
  ...
  unit_direction_ =
    (length_ <= kMathEpsilon ? Vec2d(0, 0)
                              : Vec2d(cos(heading_), sin(heading_)));
}

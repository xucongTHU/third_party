...
using apollo::math::AABoxKDTreeParams;
...
int HDMapImpl::LoadMapFromFile(const std::string& map_filename) {
  ...
  if (!loadFromFile(map_filename, &map_)) {
    return -1;
  }
...
int HDMapImpl::GetLanesWithHeading(const Vec2d& point, const double distance,
  ...
  for (auto& lane : all_lanes) {
    ...
    if (dis <= distance) {
      ...
      if (fabs(apollo::math::NormalizeAngle(heading_diff)) <=
...
int HDMapImpl::GetRoi(const apollo::geometry::PointENU& point, double radius,
...
int HDMapImpl::GetForwardNearestSignalsOnLane(
  ...
  int s_index = 0;
  apollo::math::Vec2d car_point;
  car_point.set_x(point.x());
  car_point.set_y(point.y());
  apollo::math::Vec2d map_point;
...
int HDMapImpl::GetLocalMap(const apollo::geometry::PointENU& point,
...
int HDMapImpl::GetForwardNearestRSUs(const apollo::geometry::PointENU& point,
  ...
  rsus->clear();
  LaneInfoConstPtr lane_ptr = nullptr;
  apollo::math::Vec2d target_point(point.x(), point.y());
  ...
  for (const auto& info_with_id : table) {
    ...
    for (size_t id = 0; id < info->segments().size(); ++id) {
      ...
      box_table->emplace_back(
          apollo::math::AABox2d(segment.start(), segment.end()), info,
          &segment, id);
    }
  }

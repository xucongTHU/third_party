...
int HDMap::GetLanes(const apollo::geometry::PointENU& point, double distance,
                    std::vector<LaneInfoConstPtr>* lanes) const {
  return impl_.GetLanes(point, distance, lanes);
}

int HDMap::GetJunctions(const apollo::geometry::PointENU& point, double distance,
                        std::vector<JunctionInfoConstPtr>* junctions) const {
  return impl_.GetJunctions(point, distance, junctions);
}

int HDMap::GetSignals(const apollo::geometry::PointENU& point, double distance,
                      std::vector<SignalInfoConstPtr>* signals) const {
  return impl_.GetSignals(point, distance, signals);
}

int HDMap::GetCrosswalks(const apollo::geometry::PointENU& point, double distance,
                         std::vector<CrosswalkInfoConstPtr>* crosswalks) const {
  return impl_.GetCrosswalks(point, distance, crosswalks);
}

int HDMap::GetStopSigns(const apollo::geometry::PointENU& point, double distance,
...
int HDMap::GetRoads(const apollo::geometry::PointENU& point, double distance,
...
int HDMap::GetNearestLane(const apollo::geometry::PointENU& point,
                          LaneInfoConstPtr* nearest_lane, double* nearest_s,
                          double* nearest_l) const {
  return impl_.GetNearestLane(point, nearest_lane, nearest_s, nearest_l);
}

int HDMap::GetNearestLaneWithHeading(const apollo::geometry::PointENU& point,
...
int HDMap::GetLanesWithHeading(const apollo::geometry::PointENU& point,
                               const double distance,
                               const double central_heading,
                               const double max_heading_difference,
                               std::vector<LaneInfoConstPtr>* lanes) const {
...
int HDMap::GetRoadBoundaries(
    const apollo::geometry::PointENU& point, double radius,
...
int HDMap::GetRoi(const apollo::geometry::PointENU& point, double radius,
...
int HDMap::GetLocalMap(const apollo::geometry::PointENU& point,
                       const std::pair<double, double>& range,
                       Map* local_map) const {
  return impl_.GetLocalMap(point, range, local_map);
}

int HDMap::GetForwardNearestRSUs(const apollo::geometry::PointENU& point,
...
std::vector<LaneInfoConstPtr> HDMap::GetSuccessorLanes(const Id& id) const {
  std::vector<LaneInfoConstPtr> successor_lanes;
  auto lane = impl_.GetLaneById(id);
  if (lane == nullptr) {
    return successor_lanes;
  }
  for (const auto& successor_id : lane->lane().successor_id()) {
    LaneInfoConstPtr successor_lane = GetLaneById(successor_id);
    if (successor_lane != nullptr) {
      successor_lanes.push_back(successor_lane);
    }
  }
...
std::vector<LaneInfoConstPtr> HDMap::GetPredecessorLanes(const Id& id) const {
  std::vector<LaneInfoConstPtr> predecessor_lanes;
  auto lane = impl_.GetLaneById(id);
  //AERROR << "ULT TEST: id: " << lane->lane().has_length();
  if (lane == nullptr) {
    return predecessor_lanes;
  }
  for (const auto& predecessor_id : lane->lane().predecessor_id()) {
    LaneInfoConstPtr predecessor_lane = GetLaneById(predecessor_id);
    if (predecessor_lane != nullptr) {
      predecessor_lanes.push_back(predecessor_lane);
    }
  }
...
bool HDMap::IsSuccessorLane(const std::string& cur_lane_id, 
                            const std::string& predecessor_lane_id) const {
  apollo::hdmap::Id id;
  id.set_id(cur_lane_id);
  std::vector<apollo::hdmap::LaneInfoConstPtr> lanes = GetPredecessorLanes(id);
  for (const auto& lane : lanes) {
    if (lane->id().id() == predecessor_lane_id) {
      return true;
    }
  }

...
#include "third_party/apollo/hdmap/hdmap_impl.h"
...
class HDMap {
 public:
  ...
  std::vector<LaneInfoConstPtr> GetSuccessorLanes(const Id& id) const;
  std::vector<LaneInfoConstPtr> GetPredecessorLanes(const Id& id) const;

  bool IsSuccessorLane(const std::string& cur_lane_id, 
                       const std::string& predecessor_lane_id) const;

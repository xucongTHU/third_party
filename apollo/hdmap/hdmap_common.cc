...
apollo::math::Polygon2d ConvertToPolygon2d(const Polygon &polygon) {
  ...
  while (points.size() >= 2 && points[0].DistanceTo(points.back()) <=
                                   apollo::math::kMathEpsilon) {
    points.pop_back();
  }
  return apollo::math::Polygon2d(points);
}

void SegmentsFromCurve(
    const Curve &curve,
    std::vector<apollo::math::LineSegment2d> *segments) {
...
double LaneInfo::Heading(const double s) const {
  ...
  if (index == 0 || *iter - s <= apollo::math::kMathEpsilon) {
    return headings_[index];
  }
  return apollo::math::slerp(headings_[index - 1], accumulated_s_[index - 1],
...
bool LaneInfo::IsOnLane(const apollo::math::Box2d &box) const {
...
PointENU LaneInfo::GetSmoothPoint(double s) const {
  ...
  if (delta_s < apollo::math::kMathEpsilon) {
...
void LaneInfo::CreateKDTree() {
  apollo::math::AABoxKDTreeParams params;
  ...
  for (size_t id = 0; id < segments_.size(); ++id) {
    ...
    segment_box_list_.emplace_back(
        apollo::math::AABox2d(segment.start(), segment.end()), this,
        &segment, id);
  }

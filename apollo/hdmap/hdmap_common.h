...
#include "third_party/apollo/math/aabox2d.h"
#include "third_party/apollo/math/aaboxkdtree2d.h"
#include "third_party/apollo/math/math_utils.h"
#include "third_party/apollo/math/polygon2d.h"
#include "third_party/apollo/math/vec2d.h"

#include "third_party/apollo/proto/hdmap/map_clear_area.pb.h"
#include "third_party/apollo/proto/hdmap/map_crosswalk.pb.h"
#include "third_party/apollo/proto/hdmap/map_id.pb.h"
#include "third_party/apollo/proto/hdmap/map_junction.pb.h"
#include "third_party/apollo/proto/hdmap/map_lane.pb.h"
#include "third_party/apollo/proto/hdmap/map_overlap.pb.h"
#include "third_party/apollo/proto/hdmap/map_parking_space.pb.h"
#include "third_party/apollo/proto/hdmap/map_pnc_junction.pb.h"
#include "third_party/apollo/proto/hdmap/map_road.pb.h"
#include "third_party/apollo/proto/hdmap/map_signal.pb.h"
#include "third_party/apollo/proto/hdmap/map_speed_bump.pb.h"
#include "third_party/apollo/proto/hdmap/map_stop_sign.pb.h"
#include "third_party/apollo/proto/hdmap/map_yield_sign.pb.h"
#include "third_party/apollo/proto/hdmap/map_rsu.pb.h"
...
class ObjectWithAABox {
 public:
  ObjectWithAABox(const apollo::math::AABox2d &aabox,
                  const Object *object, const GeoObject *geo_object,
                  const int id)
      : aabox_(aabox), object_(object), geo_object_(geo_object), id_(id) {}
  ~ObjectWithAABox() {}
  const apollo::math::AABox2d &aabox() const { return aabox_; }
  double DistanceTo(const apollo::math::Vec2d &point) const {
    return geo_object_->DistanceTo(point);
  }
  double DistanceSquareTo(const apollo::math::Vec2d &point) const {
...
struct LineBoundary {
  std::vector<apollo::geometry::PointENU> line_points;
};
struct PolygonBoundary {
  std::vector<apollo::geometry::PointENU> polygon_points;
};
...
using LaneSegmentBox =
    ObjectWithAABox<LaneInfo, apollo::math::LineSegment2d>;
using LaneSegmentKDTree = apollo::math::AABoxKDTree2d<LaneSegmentBox>;
...
class LaneInfo {
 public:
  ...
  const std::vector<apollo::math::Vec2d> &points() const {
    return points_;
  }
  const std::vector<apollo::math::Vec2d> &unit_directions() const {
  ...
  const std::vector<apollo::math::LineSegment2d> &segments() const {
    return segments_;
  }
  ...
  bool IsOnLane(const apollo::math::Vec2d &point) const;
  bool IsOnLane(const apollo::math::Box2d &box) const;

  apollo::geometry::PointENU GetSmoothPoint(double s) const;
  double DistanceTo(const apollo::math::Vec2d &point) const;
  double DistanceTo(const apollo::math::Vec2d &point,
                    apollo::math::Vec2d *map_point, double *s_offset,
                    int *s_offset_index) const;
  apollo::geometry::PointENU GetNearestPoint(
      const apollo::math::Vec2d &point, double *distance) const;
  bool GetProjection(const apollo::math::Vec2d &point,
 ...
 private:
  const Lane &lane_;
  std::vector<apollo::math::Vec2d> points_;
  std::vector<apollo::math::Vec2d> unit_directions_;
  std::vector<double> headings_;
  std::vector<apollo::math::LineSegment2d> segments_;
...
class JunctionInfo {
 public:
  ...
  const apollo::math::Polygon2d &polygon() const { return polygon_; }
 ...
 private:
  ...
  apollo::math::Polygon2d polygon_;

  std::vector<Id> overlap_stop_sign_ids_;
  std::vector<Id> overlap_ids_;
};
using JunctionPolygonBox =
    ObjectWithAABox<JunctionInfo, apollo::math::Polygon2d>;
using JunctionPolygonKDTree =
    apollo::math::AABoxKDTree2d<JunctionPolygonBox>;
...
using SignalSegmentBox =
    ObjectWithAABox<SignalInfo, apollo::math::LineSegment2d>;
using SignalSegmentKDTree =
    apollo::math::AABoxKDTree2d<SignalSegmentBox>;
...
using CrosswalkPolygonBox =
    ObjectWithAABox<CrosswalkInfo, apollo::math::Polygon2d>;
using CrosswalkPolygonKDTree =
    apollo::math::AABoxKDTree2d<CrosswalkPolygonBox>;
...
using StopSignSegmentBox =
    ObjectWithAABox<StopSignInfo, apollo::math::LineSegment2d>;
using StopSignSegmentKDTree =
    apollo::math::AABoxKDTree2d<StopSignSegmentBox>;
...
using YieldSignSegmentBox =
    ObjectWithAABox<YieldSignInfo, apollo::math::LineSegment2d>;
using YieldSignSegmentKDTree =
    apollo::math::AABoxKDTree2d<YieldSignSegmentBox>;
...
using ClearAreaPolygonBox =
    ObjectWithAABox<ClearAreaInfo, apollo::math::Polygon2d>;
using ClearAreaPolygonKDTree =
    apollo::math::AABoxKDTree2d<ClearAreaPolygonBox>;
...
using SpeedBumpSegmentBox =
    ObjectWithAABox<SpeedBumpInfo, apollo::math::LineSegment2d>;
using SpeedBumpSegmentKDTree =
    apollo::math::AABoxKDTree2d<SpeedBumpSegmentBox>;
...
using ParkingSpacePolygonBox =
    ObjectWithAABox<ParkingSpaceInfo, apollo::math::Polygon2d>;
using ParkingSpacePolygonKDTree =
    apollo::math::AABoxKDTree2d<ParkingSpacePolygonBox>;
...
using PNCJunctionPolygonBox =
    ObjectWithAABox<PNCJunctionInfo, apollo::math::Polygon2d>;
using PNCJunctionPolygonKDTree =
    apollo::math::AABoxKDTree2d<PNCJunctionPolygonBox>;

...
#include "third_party/apollo/proto/vehicle_state/vehicle_state.pb.h"
...
DECLARE_double(replan_lateral_distance_threshold);//from planning
DECLARE_double(replan_longitudinal_distance_threshold);//from planning
...
class PncMap {
 public:
  ...
  bool ExtendSegments(const RouteSegments &segments,
                      const geometry::PointENU &point, double look_forward,
 ...
 private:
  ...
  bool ProjectToSegments(const geometry::PointENU &point_enu,
 ...
 private:
  ...
  bool is_first_time_nearest_searching_ = true;
  std::string last_nearest_lane_id_;

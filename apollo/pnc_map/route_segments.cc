...
#include "third_party/apollo/pnc_map/route_segments.h"
...
bool RouteSegments::GetProjection(const geometry::PointENU &point_enu,
                                  geometry::SLPoint *sl_point,
...
bool RouteSegments::Shrink(const math::Vec2d &point,
  ...
  if (!GetProjection(point, &sl_point, &waypoint)) {
    ...
    std::cout << "failed to project " << point.DebugString() << " to segment";
    return false;
  }
...
bool RouteSegments::GetProjection(const math::Vec2d &point,
                                  geometry::SLPoint *sl_point,
                                  LaneWaypoint *waypoint) const {
  ...
  for (auto iter = begin(); iter != end();
    ...
    if (!iter->lane->GetProjection(point, &lane_s, &lane_l)) {
      ...
      std::cout << "Failed to get projection from point " << point.DebugString()
          << " on lane " << iter->lane->id().id();
      return false;
    }
...
bool RouteSegments::CanDriveFrom(const LaneWaypoint &waypoint) const {
  ...
  geometry::SLPoint route_sl;
  bool has_projection = GetProjection(point, &route_sl, &segment_waypoint);
  if (!has_projection) {
    std::cout << "No projection from waypoint: " << waypoint.DebugString();
  ...
  double heading_diff =
      math::AngleDiff(waypoint_heading, segment_heading);
  if (std::fabs(heading_diff) > M_PI / 2) {
    std::cout << "Angle diff too large:" << heading_diff;
    return false;
  }
  ...
  double dist = math::DistanceXY(point, segment_projected_point);
  const double kLaneSeparationDistance = 0.5;
  if (route_sl.l() < 0) {  // waypoint at right side
    if (dist >
        waypoint_left_width + segment_right_width + kLaneSeparationDistance) {
      std::cout << "waypoint is too far to reach: " << dist;
      return false;
    }

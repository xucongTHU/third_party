...
using apollo::math::Box2d;
using apollo::math::kMathEpsilon;
using apollo::math::LineSegment2d;
using apollo::math::Sqr;
using apollo::math::Vec2d;
...
void Path::InitPoints() {
  ...
  assert(accumulated_s_.size() == (std::size_t)num_points_);
  assert(unit_directions_.size() == (std::size_t)num_points_);
  assert(segments_.size() == (std::size_t)num_segments_);
}
...
void Path::InitLaneSegments() {
  ...
  assert(lane_segments_to_next_point_.size() == (std::size_t)num_segments_);
}
...
void Path::InitWidth() {
  ...
  assert(lane_left_width_.size() == (std::size_t)num_sample_points_);
  assert(lane_right_width_.size() == (std::size_t)num_sample_points_);

  assert(road_left_width_.size() == (std::size_t)num_sample_points_);
  assert(road_right_width_.size() == (std::size_t)num_sample_points_);
}
...
void Path::InitPointIndex() {
  ...
  assert(last_point_index_.size() == (std::size_t)num_sample_points_);
}
...
bool Path::GetProjection(const math::Vec2d& point, double* accumulate_s,
                         double* lateral) const {
  double distance = 0.0;
...
bool Path::GetProjection(const math::Vec2d& point, const double& backward_distance, 
                         const double& forward_distance, const int& index, 
  ...
  *min_distance = std::numeric_limits<double>::infinity();
  ...
  length = 0.0;
  for (int i = index; i < num_segments_; ++i) {
    const double distance = segments_[i].DistanceSquareTo(point);
    if (distance < *min_distance) {
      min_index = i;
      *min_distance = distance;
...
bool Path::GetProjection(const math::Vec2d& point, double* accumulate_s,
                         double* lateral, double* min_distance, int* index) const {
  if (segments_.empty()) {
    return false;
  }
  if (accumulate_s == nullptr || lateral == nullptr ||
...
bool Path::GetLaneWidth(const double s, double* lane_left_width,
  ...
  assert(lane_left_width != nullptr);
  assert(lane_right_width != nullptr);
...
bool Path::GetRoadWidth(const double s, double* road_left_width,
  ...
  assert(road_left_width != nullptr);
  assert(road_right_width != nullptr);
...
bool Path::OverlapWith(const math::Box2d& box, double width) const {
  if (use_path_approximation_) {
...
void PathApproximation::InitProjections(const Path& path) {
  ...
  for (int i = 0; i + 1 < num_original_points; ++i) {
    assert(max_original_projections_to_left_[i] <=
  ...
  for (int i = 0; i + 1 < num_original_points; ++i) {
    assert(min_original_projections_to_right_[i] <=
  ...
  assert(sampled_max_original_projections_to_left_.size() ==
            (std::size_t)num_projection_samples_);
}

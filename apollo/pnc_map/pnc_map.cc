...
#include "third_party/apollo/pnc_map/pnc_map.h"
#include "third_party/apollo/hdmap/hdmap_util.h"
#include "third_party/apollo/pnc_map/path.h"

#include "third_party/apollo/proto/hdmap/map_id.pb.h"
#include "third_party/apollo/common/point_factory.h"
...
DEFINE_double(
    replan_lateral_distance_threshold, 0.6, "from planning, to be fixed");

DEFINE_double(
    replan_longitudinal_distance_threshold, 3.5, "from planning, to be fixed");
...
using apollo::common::PointFactory;
...
bool PncMap::UpdateVehicleState(const VehicleState &vehicle_state) {
  ...
  if (!adc_state_.has_x() ||
      (math::DistanceXY(adc_state_, vehicle_state) >
    ...
    // AERROR << "PncMap::UpdateVehicleState: "
  ...
  // AERROR << "adc_waypoint_ info, lane id: " << adc_waypoint_.lane->id().id()
  //        << ", s: " << adc_waypoint_.s;
  // AERROR << "PncMap::UpdateVehicleState route_index1: " << route_index;
  ...
  // AERROR << "PncMap::UpdateVehicleState route_index2: " << route_index;
...
#include "third_party/apollo/common/util.h"
bool PncMap::IsNewRouting(const routing::RoutingResponse &prev,
  ...
  return !apollo::common::IsProtoEqual(prev, routing);
}
...
std::vector<int> PncMap::GetNeighborPassages(const routing::RoadSegment &road,
  ...
  assert(start_passage >= 0);
  assert(start_passage <= road.passage_size());
...
bool PncMap::GetRouteSegments(const VehicleState &vehicle_state,
                              ...
                              std::list<RouteSegments> *const route_segments) {
  if (!UpdateVehicleState(vehicle_state)) {  // jamesk??
  ...
  // AERROR << "adc_route_index_: " << adc_route_index_
  //        << ", road_index: " << road_index
  //        << ", passage_index: " << passage_index
  //        << ", drive_passages: " << drive_passages.size();
  for (const int index : drive_passages) {
    const auto &passage = road.passage(index);
    RouteSegments segments;
    if (!PassageToSegments(passage, &segments)) {
      std::cout << "Failed to convert passage to lane segments." << std::endl;
      continue;
    }
    ...
    if (!segments.GetProjection(nearest_point, &sl, &segment_waypoint)) {
      std::cout << "Failed to get projection from point:" 
          << nearest_point.ShortDebugString() << std::endl;
      continue;
    }
    if (index != passage_index) {
      if (!segments.CanDriveFrom(adc_waypoint_)) {
        std::cout << "You cannot drive from current waypoint to passage:" 
            << index << std::endl;
        continue;
      }
    }
    ...
    const std::string route_segment_id 
        = std::to_string(road_index) + std::string("_") + std::to_string(index);
...
bool PncMap::GetNearestPointFromRouting(const VehicleState &state,
  ...
  std::stringstream ss;
  // ss << "GetLanesWithHeading, lane size:" << lanes.size() << "\n";
  // for (const auto& lane : lanes) {
  //   ss << "  lane id: " << lane->id().id() << "\n";
  // }
  ...
  // ss.clear();
  // ss << "range_lane_ids_, size: " << range_lane_ids_.size() << "\n";
  // for (const auto& lane : range_lane_ids_) {
  //   ss << "  lane id: " << lane << "\n";
  // }  
  // std::cout << ss.str() << std::endl;

  ss.clear();
  ss << "all_lane_ids_, size: " << all_lane_ids_.size() << "\n";
  for (const auto& lane : all_lane_ids_) {
    ss << "  lane id: " << lane << "\n";
  }    
  // std::cout << ss.str() << std::endl;

  ss.clear();
  ss << "valid_lanes, size: " << valid_lanes.size() << "\n";
  ...
  // ADEBUG << "PncMap::GetNearestPointFromRouting lanes22:" <<
  // valid_lanes.size();
  ...
  for (const auto &lane : valid_lanes) {
    ...
    double distance = 0.0;
    geometry::PointENU map_point =
        lane->GetNearestPoint({point.x(), point.y()}, &distance);
    // select previous lane
    constexpr double kInJunctionDistance = 2.0;
    std::vector<hdmap::PNCJunctionInfoConstPtr> junctions;
    if (!is_first_time_nearest_searching_ &&
        last_nearest_lane_id_ == lane->id().id() &&
        hdmap_->GetPNCJunctions(point, kInJunctionDistance, &junctions) >= 0 &&
        junctions.size() > 0 && 
        lane->lane().turn() != hdmap::Lane::NO_TURN) {
      double s = 0.0;
      double l = 0.0;
    ...
    if (distance < min_distance) {     
      ...
      LaneWaypoint cur_waypoint(lane, s);
      int route_index = GetWaypointIndex(cur_waypoint);
      if (route_index < 0 ||
          route_index >= static_cast<int>(route_indices_.size())) {
        continue;
      }
  ...
  if (is_first_time_nearest_searching_ && waypoint->lane != nullptr) {
    is_first_time_nearest_searching_ = false;
  }
  if (waypoint->lane != nullptr) {
    last_nearest_lane_id_ = waypoint->lane->id().id();
  }
...
LaneInfoConstPtr PncMap::GetRoutePredecessor(LaneInfoConstPtr lane) const {
  ...
  for (const auto &lane_id : lane->lane().predecessor_id()) {
    auto predecessor_lane = hdmap_->GetLaneById(lane_id);
    if (predecessor_lane->lane().turn() ==  hdmap::Lane::NO_TURN) {
      preferred_id = lane_id;
    }
  }
...
bool PncMap::ExtendSegments(const RouteSegments &segments,
                            const geometry::PointENU &point, double look_backward,
                            double look_forward,
...
bool PncMap::ExtendSegments(const RouteSegments &segments, double start_s,
  ...
  assert(truncated_segments != nullptr);

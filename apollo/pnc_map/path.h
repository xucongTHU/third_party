...
#include "third_party/apollo/math/box2d.h"
...
struct LaneWaypoint {
  ...
  LaneWaypoint(LaneInfoConstPtr lane, const double s)
      : lane(lane), s(s) { assert(lane != nullptr); }
  LaneWaypoint(LaneInfoConstPtr lane, const double s, const double l)
      : lane(lane), s(s), l(l) { assert(lane != nullptr); }
...
struct LaneSegment {
  ...
  LaneSegment(LaneInfoConstPtr lane, const double start_s, const double end_s)
      : lane(lane), start_s(start_s), end_s(end_s) { assert(lane != nullptr); }
  LaneInfoConstPtr lane = nullptr;
  double start_s = 0.0;
  double end_s = 0.0;
...
class MapPathPoint : public math::Vec2d {
 public:
  MapPathPoint() = default;
  MapPathPoint(const math::Vec2d& point, double heading)
...
class PathApproximation {
 public:
  ...
  const std::vector<math::LineSegment2d>& segments() const {
    return segments_;
  }

  bool GetProjection(const Path& path, const math::Vec2d& point,
                     double* accumulate_s, double* lateral,
                     double* distance) const;

  bool OverlapWith(const Path& path, const math::Box2d& box,
                   double width) const;
 ...
 protected:
  ...
  std::vector<math::LineSegment2d> segments_;
...
class Path {
 public:
  ...
  bool GetNearestPoint(const math::Vec2d& point, double* accumulate_s,
                       double* lateral) const;
  bool GetNearestPoint(const math::Vec2d& point, double* accumulate_s,
                       double* lateral, double* distance) const;
  bool GetProjectionWithHueristicParams(const math::Vec2d& point,
                                        const double hueristic_start_s,
                                        const double hueristic_end_s,
                                        double* accumulate_s, double* lateral,
                                        double* min_distance) const;
  bool GetProjection(const math::Vec2d& point, double* accumulate_s,
                     double* lateral) const;
  bool GetProjection(const math::Vec2d& point, double* accumulate_s,
                     double* lateral, double* distance) const;
  bool GetProjection(const math::Vec2d& point, double* accumulate_s,
                           double* lateral, double* min_distance, int* index) const;
  bool GetProjection(const math::Vec2d& point, const double& backward_distance, 
                     const double& forward_distance, const int& index,
                     double* accumulate_s, double* lateral, double* min_distance) const;

  bool GetHeadingAlongPath(const math::Vec2d& point,
  ...
  const std::vector<math::Vec2d>& unit_directions() const {
  ...
  bool IsOnPath(const math::Vec2d& point) const;
  bool OverlapWith(const math::Box2d& box, double width) const;

  std::string DebugString() const;

  bool GetLaneSRange(const std::string& lane_id, double* s_min, double* s_max) const {
    for (unsigned int i = 0; i < lane_segments_.size(); ++i) {
      if (lane_segments_.at(i).lane->id().id() == lane_id) {
        if (i == 0) {
          *s_min = 0.0;
          *s_max = lane_accumulated_s_.at(i);
        } else {
          *s_min = lane_accumulated_s_.at(i - 1);
          *s_max = lane_accumulated_s_.at(i);
        }
        return true;
      }
 ...
 protected:
  ...
  std::vector<math::Vec2d> unit_directions_;
  double length_ = 0.0;
